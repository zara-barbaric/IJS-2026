...
mkdir -p "{main}/mg5/C_${wils}/pp_c~a/wils"

for C in $(seq 0 0.5 20); do
	if [ "$C" = "0.0" ]; then
	        C=0.01
	fi
	if [[ -d "{main}/mg5/C_${wils}/pp_c~a/wils/C_${C}" ]]; then
		rm -r "{main}/mg5/C_${wils}/pp_c~a/wils/C_${C}"
	fi
...
output {main}/mg5/C_{wils}/pp_c~a/wils/C_${C}     
launch
0
