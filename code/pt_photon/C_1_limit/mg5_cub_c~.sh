...
mkdir -p "{main}/mg5/C_${wils}/pp_c~a/pt/C_${C}_limit"

for pt in $(seq 20 5 500); do
	if [[ -d "{main}/mg5/C_${wils}/pp_c~a/pt/C_${C}_limit/pt_${pt}" ]]; then
		rm -r "{main}/mg5/C_${wils}/pp_c~a/pt/C_${C}_limit/pt_${pt}"
	fi
...
output {main}/mg5/C_${wils}/pp_c~a/pt/C_${C}_limit/pt_${pt}     
launch
0
