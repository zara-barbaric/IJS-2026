...
mkdir -p "${main}/mg5_shower/C_${wils}/pp_cee/wils"

for C in $(seq 0 0.5 30); do
	if [ "$C" = "0.0" ]; then
        	C=0.01
        fi
        if [[ -d "${main}/mg5_shower/C_${wils}/pp_cee/wils/C_${C}" ]]; then
		rm -r "${main}/mg5_shower/C_${wils}/pp_cee/wils/C_${C}"
	fi
...
output ${main}/mg5_shower/C_${wils}/pp_cee/wils/C_${C}     
launch
1
2
0
