...
mkdir -p "${main}/mg5_shower/C_${wils}/pp_c~a/wils"

for C in 80 175; do
	if [ "$C" = "0.0" ]; then
	        C=0.01
	fi
	if [[ -d "${main}/mg5_shower/C_${wils}/pp_c~a/wils/C_${C}" ]]; then
		rm -r "${main}/mg5_shower/C_${wils}/pp_c~a/wils/C_${C}"
	fi
...
output ${main}/mg5_shower/C_${wils}/pp_c~a/wils/C_${C}     
launch
1
2
0
